\
    # recon_adv.py - single z hologram reconstructor
    ...
    if __name__=="__main__":
        ...
        ap.add_argument("--outdir",default="recon_out")
        ap.add_argument("--unwrap",action="store_true")
        args=ap.parse_args()
        os.makedirs(args.outdir,exist_ok=True)
        holo=read_img(args.holo)
        U=np.sqrt(holo)
        U1=angular_spectrum(U,args.wavelength,-args.z,args.dx)
        amp,phs=np.abs(U1),np.angle(U1)
        if args.unwrap: phs=unwrap(phs)
        imageio.v2.imwrite(os.path.join(args.outdir,"amp.png"),
        ...
        imageio.v2.imwrite(os.path.join(args.outdir,"phase.png"),
            (255*pn).astype('uint8'))
        print("Reconstruction written to",args.outdir)
